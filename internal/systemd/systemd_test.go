...
// startTestBus runs a private dbus-daemon and points the bus address at it.
// Skips the test when the binary is not installed.
func startTestBus(t *testing.T) {
	...
	//nolint:gosec // fixed binary name; only the socket path varies
	...
	// The daemon prints its address once it is listening.
	if _, err := bufio.NewReader(stdout).ReadString('\n'); err != nil {
		t.Fatalf("dbus-daemon did not report an address: %v", err)
	}
