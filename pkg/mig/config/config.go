...
func resolveMigProfileOnDevice(nvmllib nvml.Interface, device nvml.Device, mp *types.MigProfile) (*types.MigProfile, error) {
	devLib := nvdevlib.New(nvmllib)
	nvdevDevice, err := devLib.NewDevice(device)
	if err != nil {
		return nil, fmt.Errorf("error wrapping device for MIG profile resolution: %w", err)
	}
	profiles, err := nvdevDevice.GetMigProfiles()
	if err != nil {
		return nil, fmt.Errorf("error listing MIG profiles on device: %w", err)
	}
	key := mp.String()
...
func (m *nvmlMigConfigManager) SetMigConfig(gpu int, config types.MigConfig) error {
	...
	err = iteratePermutationsUntilSuccess(config, func(mps []*types.MigProfile) error {
		...
		for _, mp := range mps {
			resolved, err := resolveMigProfileOnDevice(m.nvml, device, mp)
			if err != nil {
				return err
			}
